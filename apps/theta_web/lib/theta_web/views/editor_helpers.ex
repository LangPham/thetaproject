defmodule ThetaWeb.EditorHelpers do
  ...
  def simplemde(form, field) do
    ...
    content_tag :div, class: "easy-mde" do
      ...
      script_inline =
        raw(
          "<script type=\"text/javascript\">editor.create_editor(\"" <> element <> "\")</script>"
        )
      ...
      img_select = raw("<input style='display:none' type='text' id='imgSelect' >")
  ...
  defp check_field_error(error_html, class, opt \\ []) do
    ...
    opt_in =
      if List.keymember?(opt, :class, 0) do
  ...
  def filter_input(id, column, placeholder \\ 'Search') do
    ...
    field_html =
      Phoenix.HTML.Form.text_input(:search, String.to_atom(atom_key),
        onkeyup: table_column,
